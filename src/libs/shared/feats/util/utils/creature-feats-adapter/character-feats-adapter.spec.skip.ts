...
// TODO: rewrite to test CharacterFeatsAdapter
fdescribe('featsStore', () => {
    ...
    const setup = ({ countAsFeat, superType, levelNumber }: { countAsFeat?: string; superType?: string; levelNumber: number }): {
        gain: FeatGain;
        feat: Feat;
        addAction: { feat: Feat; gain: FeatGain; levelNumber: number; temporary: boolean };
        removeAction: { gain: FeatGain; levelNumber: number };
    } => {
        const name = 'name';
        const feat = new Feat().with({ name, countAsFeat, superType }, mockRecastFns());
        // countAsFeat on the gain is normally set at runtime from the countAsFeat OR the superType property of the feat.
        const gain = new FeatGain(mockRecastFns({ feat })).with({ name });
