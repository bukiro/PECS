...
export class Activity implements Serializable<Activity> {
     ...
     */
    public readonly effectiveCooldownByCreature$$ = new WeakMap<Creature, Signal<number>>();
    /**
     * effectiveMaxChargesByCreature$ is a map of calculated cooldown signals matched to creatures
     * created by the ActivityPropertiesService so that it can be subscribed to without passing parameters.
     */
    public readonly effectiveMaxChargesByCreature$$ = new WeakMap<Creature, Signal<number>>();

    public static from(values: MaybeSerialized<Activity>, recastFns: RecastFns): Activity {
        return new Activity().with(values, recastFns);
    }

    public with(values: MaybeSerialized<Activity>, recastFns: RecastFns): this {
    ...
    public clone(recastFns: RecastFns): this {
        return Activity.from(this, recastFns) as this;
    }
    ...
    public canActivate$$(creature: Creature): Signal<boolean> {
        ...
        return computed(() =>
            stringsIncludeCaseInsensitive(this.traits, 'Stance')
            ...
            || !!this.toggle
            || !!this.effectiveMaxChargesByCreature$$.get(creature)?.()
            || !!this.effectiveCooldownByCreature$$.get(creature)?.(),
        );
    }
