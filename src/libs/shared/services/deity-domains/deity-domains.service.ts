...
export class DeityDomainsService {
    ...
    public effectiveAlternateDomains(deity: Deity, character: Character): Array<string> {
        ...
        if (!deity.$alternateDomains.length) {
            this._recreateAlternateDomains(deity, character);
        }

        return deity.$alternateDomains;
    }

    private _recreateAlternateDomains(deity: Deity, character: Character): void {
