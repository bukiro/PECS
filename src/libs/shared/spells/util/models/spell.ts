...
import { activityTargetOption, ActivityTargetOption } from 'src/libs/shared/activities/util/models/activity-target-options';
...
export class Spell implements Serializable<Spell> {
    ...
    public with(values: MaybeSerialized<Spell>, recastFns: RecastFns): this {
    ...
    public clone(recastFns: RecastFns): this {
        return Spell.from(this, recastFns) as this;
    }
    ...
    public heightenedConditions(levelNumber: number = 0): Array<ConditionGain> {
        ...
        for (
            let remainingLevelNumber = Math.max(levelNumber, this.levelreq);
            remainingLevelNumber > 0;
            remainingLevelNumber--
        ) {
            if (this.gainConditions.some(gain => gain.heightenedFilter === remainingLevelNumber)) {
                return this.gainConditions.filter(gain =>
                    matchNumberFilter({ value: remainingLevelNumber, match: gain.heightenedFilter }),
                );
            }
        }

        // If no conditions with a heightenedFilter were found at the given level,
        // return all conditions without a filter.
    ...
    public isCantrip(): boolean {
        return stringsIncludeCaseInsensitive(this.traits, 'Cantrip');
    }
