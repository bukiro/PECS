...
import { strikingTitleFromLevel } from './rune-utils';
...
export class Weapon extends Equipment implements MessageSerializable<Weapon> {
     ...
     * Use 0 to notate exactly <dicesize> damage (e.g. 1 damage with dicenum 0 and dicesize 1).
     */
    public dicenum = 1;
    /** Size of the damage dice: usually 4-12, but can be 0, 1, etc. */
    public dicesize: number = DiceSizes.D6;
    ...
    public readonly typeDescription$$ = signal(
        [
            this.prof.split(' ')[0],
            this.group,
        ].filter(part => part)
    ...
    public readonly weaponMaterial$$: Signal<Array<WeaponMaterial>> = computed(() =>
        this.material().filter(material => material.isWeaponMaterial()),
    );
    public readonly weaponRunes$$: Signal<Array<WeaponRune>> = computed(() =>
        this.propertyRunes().filter(rune => rune.isWeaponRune()),
    );
    ...
    public readonly secondaryRuneTitleFunction: ((secondary: BasicRuneLevels) => StrikingRuneLevelNames) = strikingTitleFromLevel;
    ...
    protected readonly _bladeAllyName$$: Signal<Array<string>> = computed(() => {
    ...
    public with(values: MaybeSerialized<Weapon>, recastFns: RecastFns): this {
    ...
    public clone(recastFns: RecastFns): this {
        return Weapon.from(this, recastFns) as this;
    }
