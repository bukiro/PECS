...
import { safeParseInt, stringsIncludeCaseInsensitive } from 'src/libs/shared/common/util/utils/string-utils';
...
import { MaybeSerialized, Serialized } from 'src/libs/shared/serialization/util/models/serializable';
...
export abstract class Equipment extends Item {
    ...
    public readonly potencyRune = signal<BasicRuneLevels>(0);
    /** Striking Rune level for weapons. */
    public readonly strikingRune = signal<BasicRuneLevels>(0);
    /** Resilient Rune level for armor. */
    public readonly resilientRune = signal<BasicRuneLevels>(0);
    ...
    public readonly effectiveBulk$$ = computed<Bulk>(() => {
        ...
        return this.material()
            ...
            .map(material => {
                const modifiedBulk = ownBulk + safeParseInt(material.bulkModifier, 0);
    ...
    /** Generate the item type subtitle, e.g. "Light Composite Armor" */
    public readonly typeDescription$$: Signal<string> = computed(() => '');

    /** Return the highest value of your potency rune or any oils that emulate one. */
    public readonly effectivePotency$$ = computed<BasicRuneLevels>(() =>
        Math.max(...this.oilsApplied().map(oil => oil.potencyEffect), this.potencyRune()) as BasicRuneLevels,
    );

    /** Return the highest value of your striking rune or any oils that emulate one. */
    public readonly effectiveStriking$$ = computed<BasicRuneLevels>(() =>
        Math.max(...this.oilsApplied().map(oil => oil.strikingEffect), this.strikingRune()) as BasicRuneLevels,
    );

    /** Return the highest value of your resilient rune or any oils that emulate one. */
    public readonly effectiveResilient$$ = computed<BasicRuneLevels>(() =>
        Math.max(...this.oilsApplied().map(oil => oil.resilientEffect), this.resilientRune()) as BasicRuneLevels,
    );
    ...
    public with(values: MaybeSerialized<Equipment>, recastFns: RecastFns): this {
