...
import { shoddyPenalties, ShoddyPenalties } from './shoddy-penalties';
...
import { ResilientRuneLevelNames } from './rune-level-names';
import { resilientTitleFromLevel } from './rune-utils';
...
export class Armor extends Equipment implements MessageSerializable<Armor> {
     ...
     */
    public readonly effectiveArmoredSkirtModifier$$ = signal<-1 | 0 | 1>(0);
    ...
    public readonly effectiveProficiencyWithoutEffects$$ = computed(() => {
        const armoredSkirtModifier = this.effectiveArmoredSkirtModifier$$();
    ...
    public readonly effectiveBulk$$ = computed<Bulk>(() => {
        ...
        // TODO: Can armors never have Bulk 'L'? If they can, it needs to be preserved here.
        return bulk + fortificationBulk;
    });

    public readonly typeDescription$$ = computed(() => {
        const proficiency = this.effectiveProficiencyWithoutEffects$$();
    ...
    public readonly effectiveACBonus$$ = computed(() =>
        this.effectiveArmoredSkirtModifier$$() + this.acbonus,
    );

    public readonly effectiveSkillPenalty$ = computed(() => {
        const armoredSkirtModifier = this.effectiveArmoredSkirtModifier$$();
    ...
    public readonly effectiveSpeedPenalty$$ = computed(() =>
        Math.min(
            0,
            (
                this.speedpenalty +
    ...
    public readonly effectiveDexCap$$ = computed(() => {
        const armoredSkirtModifier = this.effectiveArmoredSkirtModifier$$();
    ...
    public readonly effectiveStrengthRequirement$$ = computed(() => {
        const armoredSkirtModifier = this.effectiveArmoredSkirtModifier$$();
    ...
    public readonly secondaryRuneTitleFunction: ((secondary: BasicRuneLevels) => ResilientRuneLevelNames) = resilientTitleFromLevel;
    ...
    public with(values: MaybeSerialized<Armor>, recastFns: RecastFns): this {
    ...
    public clone(recastFns: RecastFns): this {
        return Armor.from(this, recastFns) as this;
    }
    ...
    // TODO: This doesn't need to be here. Put it where it's used (the item component or the inventory component)
