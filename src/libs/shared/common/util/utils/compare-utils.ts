import { Serializable } from '../../../serialization/util/models/serializable';
...
export const isEqualSerializableArray = <T extends Serializable<unknown>>(a: Array<T>, b: Array<T>): boolean =>
    a.length === b.length
    && b.every((_, index) => !!a[index] && !!b[index] && b[index].isEqual(a[index]));

export const isEqualSerializableArrayWithoutId = <T extends Serializable<unknown>>(a: Array<T>, b: Array<T>): boolean =>
    a.length === b.length
    && b.every((_, index) => !!a[index] && !!b[index] && b[index].isEqual(a[index], { withoutId: true }));
...
export const isEqualObjectArray = <T>(compareFn: (a: T, b: T) => boolean): (a: Array<T>, b: Array<T>) => boolean =>
    (a: Array<T>, b: Array<T>) =>
        a.length === b.length
        && b.every((_, index) => !!a[index] && !!b[index] && compareFn(a[index], b[index]));
