export const sortAlphaNum = <T extends string | number>(prev: T, next: T): number => (prev === next) ? 0 : ((prev > next) ? 1 : -1);

export const sortAlphaNumDesc = <T extends string | number>(prev: T, next: T): number => (prev === next) ? 0 : ((prev < next) ? 1 : -1);
