import { Serializable, MessageSerializable, MaybeSerialized, Serialized } from '../models/serializable';
import { RecastFns } from '../models/recast-fns';
import { forEachMember } from '../../../common/util/utils/object-utils';
import { isSignal, Signal, WritableSignal } from '@angular/core';
import { isWritableSignal, ResolveSignal } from '../../../common/util/utils/signal-utils';

type AssignFn<T, P> = (parent: P) => (obj: T | MaybeSerialized<T>, index?: number) => T;
type AssignFnWithHelpers<T, P> = (recastFns: RecastFns, parent: P) => (obj: T | MaybeSerialized<T>, index?: number) => T;
...
type SerializableSet<T> = Partial<{
    [K in SerializableKey<T>]: AssignFn<ResolveSignal<T[K]>, T>;
}>;

type SerializableSetWithHelpers<T> = Partial<{
    [K in SerializableKey<T>]: AssignFnWithHelpers<ResolveSignal<T[K]>, T>;
}>;

type SerializableArraySet<T> = Partial<{
    [K in AnySerializableArrayKey<T>]: AssignFn<ResolveSignal<ArrayMemberType<T, K>>, T>;
}>;

type SerializableArraySetWithHelpers<T> = Partial<{
    [K in AnySerializableArrayKey<T>]: AssignFnWithHelpers<ResolveSignal<ArrayMemberType<T, K>>, T>;
}>;
...
namespace forImport {
    ...
    export const serializableProperty =
        <T extends object, K extends SerializableKey<T>>(
            ...
            values: Partial<T> | MaybeSerialized<T>,
            key: K,
            assignFn: AssignFn<T[K], T>,
        ): void => {
            const value = (values as T)[key];

            if (value) {
                applyValue(obj, key, value, innerValue => assignFn(obj)(innerValue));
            }
        };
    ...
    export const serializablePropertyWithHelpers =
        <T extends object, K extends SerializableKey<T>>(
            ...
            values: Partial<T> | MaybeSerialized<T>,
            key: K,
            assignFn: AssignFnWithHelpers<T[K], T>,
            recastFns: RecastFns,
        ): void => {
            const value = (values as T)[key];

            if (value) {
                applyValue(obj, key, value, innerValue => assignFn(recastFns, obj)(innerValue));
            }
        };
    ...
    export const serializableArrayProperty =
        <T extends object, K extends AnySerializableArrayKey<T>>(
            ...
            values: Partial<T> | MaybeSerialized<T>,
            key: K,
            assignFn: AssignFn<ArrayMemberType<T, K>, T>,
        ): void => {
            const value = (values as T)[key];
    ...
    export const serializableArrayPropertyWithHelpers =
        <T extends object, K extends AnySerializableArrayKey<T>>(
            ...
            values: Partial<T> | MaybeSerialized<T>,
            key: K,
            assignFn: AssignFnWithHelpers<ArrayMemberType<T, K>, T>,
            recastFns: RecastFns,
        ): void => {
            const value = (values as T)[key];
