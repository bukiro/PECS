...
export class ConditionComponent extends TrackByMixin(BaseClass) {
    ...
    public selectOtherConditionOptions$(selection: OtherConditionSelection, gain: ConditionGain, index: number): Observable<Array<string>> {
        ...
        const allowedConditions = this._conditionsDataService.conditions().filter(libraryCondition =>
            matchStringFilter({ match: selection.typeFilter, value: libraryCondition.type })
            && matchStringFilter({ match: selection.nameFilter, value: libraryCondition.name }),
        )
                ...
                map(conditions =>
                    conditions
                        ...
                        .filter(conditionName =>
                            ...
                            && matchStringFilter({ match: allowedConditions, value: conditionName }),
                        )
