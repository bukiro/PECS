...
import { EffectGain } from '../../../../../libs/shared/effects/util/models/effect-gain';
...
export class ObjectEffectsComponent {
    ...
    public readonly objectName$$ = input<string>('', { alias: 'objectName' });

    public readonly creature$$ = input<Creature>(CreatureService.character$$(), { alias: 'creature' });

    public readonly customEffectsOnThis = computed(() => {
        const objectName = this.objectName$$();

        return this.creature$$()
            .effects()
            .filter(effect => stringEqualsCaseInsensitive(effect.affected, objectName));
    });

    public readonly bonusTypes = Object.values(bonusTypes).map(type => type === bonusTypes.untyped ? '' : type);
