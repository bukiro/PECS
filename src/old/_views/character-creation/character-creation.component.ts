...
import { replaceFirstMemberFromArrayWhere } from 'src/libs/shared/common/util/utils/array-utils';
        ...
        this.familiar$
            .pipe(
                map(familiar => {
                    ...
                    const landSpeed = 'Land Speed';
                    const swimSpeed = 'Swim Speed';

                    const oldSpeed = isChecked
                        ? landSpeed
                        : swimSpeed;

                    const newSpeed = isChecked
                        ? swimSpeed
                        : landSpeed;

                    familiar.speeds = replaceFirstMemberFromArrayWhere(
                        familiar.speeds,
                        ({ name }) => stringEqualsCaseInsensitive(name, oldSpeed),
                        old => Speed.from({ ...old, name: newSpeed }),
                    );
