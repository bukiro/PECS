    ...
    get_WeaponPropertyRunes(weapon: Equipment, number: number) {
        if (this.itemStore) {
            ...
            .filter(
                ...
                ((rune.traits.indexOf("Saggorak") > -1) ? (weapon.potencyRune > 1 && weapon.propertyRunes.filter(rune => rune == "").length > (3 - weapon.potencyRune)) : true)
            ).map(rune => rune.name));
        } else {
    ...
    onWeaponRuneChange(runeType:string, weapon: Equipment, runeSlot: number, previousRune: number|string) {
        switch (runeType) {
            case "potency":
                ...
                //If the potency rune has been lowered, but the striking rune hasn't, throw out the striking rune
                ...
                if (weapon.potencyRune < previousRune) {
                    //Clear all property rune slots that have become invalid
                ...
                if (previousRune != weapon.propertyRunes[runeSlot]) {
                    ...
                    if (previousRune != "" && (previousRune as string).substr(0,6) != "Locked") {
                        ...
                        if (extractedRune.traits.indexOf("Saggorak") > -1) {
                            ...
                            if (lockedSlots.length) {
                                ...
                                if (weapon.propertyRunes[0].substr(0,6) == "Locked") {
                                    weapon.propertyRunes[0] = "";
                                    done = true;
                                }
                                if (weapon.propertyRunes[1].substr(0,6) == "Locked" && !done) {
                                    weapon.propertyRunes[1] = "";
                                    done = true;
                                }
                                if (weapon.propertyRunes[2].substr(0,6) == "Locked" && !done) {
                                    weapon.propertyRunes[2] = "";
                                    done = true;
                                }
                            }
                        }
                    ...
                    if (weapon.propertyRunes[runeSlot]) {
                        ...
                        if (exampleRune.traits.indexOf("Saggorak") > -1) {
                            ...
                            if (freeSlots.length) {
                                let done: boolean = false;
                                if (weapon.propertyRunes[0] == "") {
                                    weapon.propertyRunes[0] = "Locked by "+weapon.propertyRunes[runeSlot];
                                    done = true;
                                }
                                if (weapon.propertyRunes[1] == "" && !done) {
                                    weapon.propertyRunes[1] = "Locked by "+weapon.propertyRunes[runeSlot];
                                    done = true;
                                }
                                if (weapon.propertyRunes[2] == "" && !done) {
                                    weapon.propertyRunes[2] = "Locked by "+weapon.propertyRunes[runeSlot];
                                    done = true;
                                }
                            }
                        }
